...
public readonly struct ChannelStatistics
{
    ...
    internal const string PropPrefix = "omx.stats.";
    internal const string PropCount = "omx.stats.count";
    internal const string PropMin = "omx.stats.min";
    internal const string PropMax = "omx.stats.max";
    internal const string PropSum = "omx.stats.sum";
    internal const string PropMean = "omx.stats.mean";
    internal const string PropVariance = "omx.stats.variance";
    internal const string PropFirst = "omx.stats.first";
    internal const string PropLast = "omx.stats.last";

    internal void WriteToProperties(Dictionary<string, OmxValue> props)
    {
        if (Count == 0) return;
    ...
    internal static ChannelStatistics? ReadFromProperties(IReadOnlyDictionary<string, OmxValue> props)
    {
...
internal static class NumericConverter
{
    ...
    public static double ToDouble<T>(T value) where T : unmanaged
    {
        ...
        if (typeof(T) == typeof(OmxTimestamp)) return Unsafe.As<T, OmxTimestamp>(ref value).Nanoseconds;
